...
CORS(
    app, resources=r"/api/*", allow_headers=["Content-Type"], supports_credentials=True
)
...
def checkLoggedIn():
    try:
        ...
        cursor = cnx.cursor()
        query = "SELECT email, username, firstName, lastName FROM users WHERE token=%s"
        data = [token]
...
def login_user():
    ...
    cnx = get_db_connection()
    cursor = cnx.cursor()
    query = "SELECT username, email, firstname, lastname, password FROM users WHERE username=%s OR email=%s"
    ...
    if pwhash:
        ...
        try:
            ...
            token = sha256(str(uuid4()).encode("utf-8")).hexdigest()
            # add token to db
            query = "UPDATE users SET token=%s WHERE username=%s OR email=%s"
...
def logout_user():
    ...
    if token:
        ...
        return (
            ...
            200,
            {"Set-Cookie": f"token=0; max-age=0"},
        )
    else:
...
def get_streaks():
    ...
    try:
        ...
        query = "SELECT * FROM streaks WHERE userID=%s"
        data = [userID]
        cursor.execute(query, data)
...
def create_streak():
    ...
    if token:
        ...
        # add streak to db
        query = "INSERT INTO streaks VALUES (%s, %s)"
...
def delete_streak():
    ...
    if token:
        ...
        query = "DELETE FROM streak_history WHERE streakID=%s"
        data = (streakID,)
        cursor.execute(query, data)
        cnx.commit()

        query = "DELETE FROM streaks WHERE id=%s"
        cursor.execute(query, data)
        cnx.commit()
        cursor.close()
        cnx.close()
...
def markMultipleDone():
    ...
    if errors:
        return {
            "message": "1 or more streaks were already done for 1 or more of the selected dates",
            "failed": errors,
        }, 400
...
def markMultipleNotDone():
    ...
    streakIDs = request.json["ids"]
    streakDates = request.json["dates"]
    query = "DELETE FROM streak_history WHERE streakID=%s AND date=%s"
    errors = []
    for streakID in streakIDs:
        for streakDate in streakDates:
